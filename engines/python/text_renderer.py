...
def render(
    ...
    *,
    ...
    concealed_positions = observation_concealed_positions(state, effective_game)
...
def _ordered_layers(state: GameState, game_def) -> list[str]:
    return order_layer_ids(list(state.board.layers), game_def)


def order_layer_ids(layer_ids: list[str], game_def) -> list[str]:
    """Return ``layer_ids`` from visually topmost to bottommost.

    The DSL declares layers bottom-to-top. Deriving observation priority from
    that declaration keeps the text grid, the target grid and the board image
    aligned with the Flutter board instead of imposing game-specific layer
    names. Layers the game does not declare sit above every declared layer,
    in the order given (last on top).
    """
    declared = [
        layer["id"]
    ...
    remaining = [
        layer for layer in layer_ids if layer not in declared_set
    ]
    return list(reversed([*declared, *remaining]))


def observation_concealed_positions(state: GameState, game_def) -> set[Pos]:
