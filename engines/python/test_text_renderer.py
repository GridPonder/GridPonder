...
def test_board_unchanged_note_survives_a_system_status_block() -> None:
    # The runner renders the previous board with level_def, so a system
    # status block is part of it; the prompt must compare against a render
    # with the same inputs or an unchanged board always looks changed.
    game = _make_elastic_game()
    level = _make_elastic_level()
    engine = TurnEngine(game, level)
    before = text_renderer.render(
        engine.state, game, include_legend=False, level_def=level
    )
    assert "completed, still occupied by Bellows" in before
    prompt = build_prompt(
        game, level, engine.state,
        last_action={"action": "move", "direction": "up"},
        previous_board_text=before,
    )
    assert "The board did not change." in prompt, prompt
