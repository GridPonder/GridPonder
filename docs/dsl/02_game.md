...
stacked-cell detail, per-entity state, system status blocks, and goal text
print that kind's `uiName` (or id) and `description`, whichever member is on
the board. Declare the group's neutral public face first, for example a kind
